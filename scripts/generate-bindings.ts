...
// Rootless roots also get no render prop — there is no element for a consumer
// to replace, and Base UI has no notion of a render prop on a fragment root.
const fragmentBody = '<>{children}</>';

// Every generated element renders through binding-runtime's shared Part frame,
// which owns the render-prop branch, presence gate, and composed ref — the
// declaration site only states what varies: tag, options-or-merged, flags, and
// children. `attrs` carries either `options={{...}}` (Part runs usePartProps)
// or `merged={...}` (a bespoke body precomputed the Tracked the element spreads
// — the same object backs the render prop's live proxy). Void tags self-close:
// Part's childless-element form keys off the absent children block. Callers
// wrap the result in whatever gate they need (file-upload's `@if (url.value)`);
// the presence gate itself is Part's own `gate` flag.
function partFrame(tag: string, attrs: string, childrenExpr: string = '{children}') {
	const open = `<Part tag="${tag}" ${attrs} of={props}`;
	return voidTags.has(tag) ? `${open} />` : `${open}>${childrenExpr}</Part>`;
}

function renderableElement(tag: string | undefined, propsExpr: string) {
	if (!tag) return fragmentBody;
	return partFrame(tag, `merged={${propsExpr}}`);
}
...
function presencePartDeclaration(
	...
	${partFrame(part.tag, 'merged={mergedProps}')}
		...
		return `export function ${part.partName}(props: ${part.partName}Props) @{
	let &{ children } = props;
	${partFrame(part.tag, `options={{ ${fields.join(', ')} }}`)}
}`;
	}

	// Gate/ref/merge live in binding-runtime's Part frame; this site only maps the
	// upstream behavior table onto its flags. `omitKeys: ['ref']` pairs with
	// composeRef: the user's ref must stay out of localProps so Part's composed
	// ref (setNode + user ref) is the sole ref reaching the element.
	const fields = [
		`context`,
		part.getter ? `getter: '${part.getter}'` : undefined,
		part.keys.length ? `propKeys: ${literal(part.keys)}` : undefined,
		behavior.mergePresence ? `presence: presenceContext` : undefined,
		behavior.presenceRef ? `omitKeys: ['ref']` : undefined,
	].filter(Boolean);
	const flags = [
		'presence={presenceContext}',
		behavior.gate ? 'gate' : undefined,
		behavior.presenceRef ? 'composeRef' : undefined,
	].filter(Boolean);
	return `export function ${part.partName}(props: ${part.partName}Props) @{
	let &{ children } = props;
...
function partDeclaration(
	...
	const setup = [options?.needsApi ? 'const api = context.get()!;' : undefined, options?.prelude]
		.filter(Boolean)
		.join('\n\t');
	// `content` is a children *expression* handed to the Part frame (e.g.
	// `{children || api.value.xxx}`); a render consumer replaces the element
	// wholesale, so it never sees the derived-content fallback — only the plain
	// element does. Part owns that branch.
	const childrenExpr = options?.content ? options.content(effectivePart) : '{children}';
	const isVoid = voidTags.has(effectivePart.tag);
	return `export function ${effectivePart.partName}(props: ${effectivePart.partName}Props) @{
	${isVoid ? '' : 'let &{ children } = props;\n\t'}${setup ? setup + '\n\t' : ''}${partFrame(effectivePart.tag, `options={{ ${fields.join(', ')} }}`, childrenExpr)}
}`;
}
...
const partOverrides: Record<string, Record<string, string | ((part: PartSpec) => string)>> = {
	...
	const mergedProps = track(() => ({ role: 'group', ...splitProps(props, ${literal(part.keys)})[1], ...checkboxGroupAnatomyAttrs }));
	${partFrame(part.tag, 'merged={mergedProps}')}
}`,
	...
	@if (url.value) {
		...
		ItemName: (part) =>
			partDeclaration(part, {
				...
				content: () => `{children || itemProps.value.file.name}`,
			}),
		ItemSizeText: (part) =>
			partDeclaration(part, {
				needsApi: true,
				prelude: 'const itemProps = fileUploadItemProps.get()!;',
				content: () => `{children || api.value.getFileSize(itemProps.value.file)}`,
			}),
	},
	...
	'angle-slider': {
		ValueText: (part) =>
			partDeclaration(part, {
				needsApi: true,
				content: () => `{children || api.value.valueAsDegree}`,
			}),
	},
	clipboard: {
		ValueText: (part) =>
			partDeclaration(part, {
				needsApi: true,
				content: () => `{children || api.value.value}`,
			}),
	},
	'color-picker': {
		ValueText: (part) =>
			partDeclaration(part, {
				needsApi: true,
				content: () => `{children || ((props as any).format
					? api.value.value.toString((props as any).format)
					: api.value.valueAsString)}`,
			}),
	},
	listbox: {
		ValueText: (part) =>
			partDeclaration(part, {
				...
				content: () => `{children || api.value.valueAsString || (props as any).placeholder}`,
			}),
	},
	progress: {
		ValueText: (part) =>
			partDeclaration(part, {
				needsApi: true,
				content: () => `{children || api.value.percentAsString}`,
			}),
	},
	slider: {
		ValueText: (part) =>
			partDeclaration(part, {
				needsApi: true,
				content: () => `{children || api.value.value.join(',')}`,
			}),
		DraggingIndicator: (part) =>
			partDeclaration(part, {
				needsApi: true,
				prelude: 'const thumbProps = sliderThumbProps.get()!;',
				content: () => `{children || api.value.getThumbValue(thumbProps.value.index)}`,
			}),
	},
	...
	${partFrame(part.tag, 'merged={mergedProps}', '{api.value.valueAsString || (props as any).placeholder}')}
}`,
	},
	...
	${partFrame(part.tag, 'merged={mergedProps}', '{(api.value.formattedTime as any)[(props as any).type]}')}
}`,
	},
	carousel: {
	...
	const mergedProps = track(() => ({ ...carouselProgressTextAttrs, ...localProps }));
	${partFrame(part.tag, 'merged={mergedProps}', '{children || progressText.value}')}
}`,
	},
	tour: {
		...
		ActionTrigger: (part) =>
			partDeclaration(part, {
				content: () => `{children || (props as any).action?.label}`,
			}),
		ProgressText: (part) =>
			partDeclaration(part, {
				needsApi: true,
				content: () => `{children || api.value.getProgressText()}`,
			}),
		Title: (part) =>
			partDeclaration(part, {
				needsApi: true,
				content: () => `{children || api.value.step?.title}`,
			}),
		Description: (part) =>
			partDeclaration(part, {
				needsApi: true,
				content: () => `{children || api.value.step?.description}`,
			}),
	},
};
...
const rootOverrides: Record<string, (configKeys: string[]) => string> = {
	...
	${partFrame('label', 'merged={mergedProps}')}
}`,
...
function drawerIndentDeclaration(part: PartSpec, getter: string) {
	...
	${partFrame('div', 'merged={mergedProps}')}
}`;
}
...
for (const directory of readdirSync(solidComponents, { withFileTypes: true })) {
	...
	const bindingRuntimeImports = [
		...
		...(allBodyText.includes('useExternalPartProps(') ? ['useExternalPartProps'] : []),
		...(allBodyText.includes('usePartProps(') ? ['usePartProps'] : []),
